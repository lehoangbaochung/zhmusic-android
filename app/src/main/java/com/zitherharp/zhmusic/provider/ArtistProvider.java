...
public class ArtistProvider {
    ...
    private static String getSongLoaderSortOrder() {
        return MediaStore.Audio.Artists.DEFAULT_SORT_ORDER + ", " + MediaStore.Audio.Albums.DEFAULT_SORT_ORDER
    ...
    private static List<Artist> getAllArtists(@NonNull final Context context) {
        List<Song> songs = SongProvider.getSongs(SongProvider.makeSongCursor(context, getSongLoaderSortOrder()));
    ...
    private static Artist getOrCreateArtist(List<Artist> artists, String artistId) {
        for (Artist artist : artists) {
            if (!artist.albums.isEmpty() && !artist.albums.get(0).songs.isEmpty()
                    && artist.albums.get(0).songs.get(0).artistId == artistId) {
                return artist;
            }
        }
